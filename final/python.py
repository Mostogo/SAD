...
haut_moy_tab = array([0,0,0])
bas_moy_tab = array([0,0,0])
...
def calcul_moy(valHaut, valBas):
    ...
    somme_haut = 0
    somme_bas = 0
    haut_moy_tab[cptPos%3] = valHaut
    bas_moy_tab[cptPos%3] = valBas
    cptPos = cptPos + 1
    for i in range(0, 2):
        somme_haut = somme_haut + haut_moy_tab[i]
        
    for j in range(0, 2):
        somme_bas = somme_bas + bas_moy_tab[i]
    haut_moy = int(somme_haut/3)
    bas_moy = int(somme_bas/3)
...
while True:
    ...
    # Centre camera
    middleScreenX = int(resolutionX/2) +50
    middleScreenY = int(resolutionY/2) - 20
 ...
 # find contours in the mask and initialize the current
    ...
    if len(cnts) > 0:
        ...
        if radius > minObjectSize:
            ...
            messageDistance = str(radius) + "..." + "x = " + str(valeurBas) + "..." + str(bas_moy) +",    y = " + str(valeurHaut) + "..." + str(haut_moy)
 ...
 # loop over the set of tracked points
    ...
    message="/"+str(haut_moy)+"/"+str(bas_moy)+"/"+str(tirer)+"/"+str(radius)
