...
serial_port = serial.Serial(port='COM6', baudrate=9600, timeout=0.1)
...
while True:
 ...
 # loop over the set of tracked points
        ...
        if not ((middleScreenX == centerObjectX) & (middleScreenY == centerObjectY)):
            if not (middleScreenX == centerObjectX):
                if (centerObjectX < middleScreenX):
                    valeurBas = int((centerObjectX - middleScreenX)/10)
                    print(int(valeurBas))
                    #print("GAUCHE")
                else:
                    valeurBas = int((centerObjectX - middleScreenX)/10)
            ...
            if not (middleScreenY == centerObjectY):
                if (centerObjectY < middleScreenY):
                    valeurHaut = int((middleScreenY - centerObjectY)/10)
    ...
    message=str(valeurBas)+"/"+str(valeurHaut)+"/"+str(tirer)+"/"+str(radius)
